...
// Grainient — React Bits component (JavaScript + CSS variant), ported to TSX.
// WebGL2 warped tri-color gradient with film grain. Renderer is built once and
// kept alive across re-renders (ctxMap) so prop changes only update uniforms.
...
// can update values without ever rebuilding the WebGL context
...
const Grainient = ({
  ...
  // effect 2: sync props to uniforms — zero GPU rebuild cost
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
